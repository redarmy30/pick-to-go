...
//Low level pid target values are set here__________________________________
  ...
  for(i =0; i < 2; i++)
  {
    ...
    wheelsPidStruct[0].current = telega.rightwheel.speed; // текущие занчения скоростей колес
    wheelsPidStruct[1].current = telega.leftwheel.speed; // текущие занчения скоростей колес
