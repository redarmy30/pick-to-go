...
void pidCalc1(PidStruct *pid_control)  //рассчет ПИД
{
  ...
  pid_control->output = pid_control->output ;
}
...
void initRegulators1(void){  // инициализация регуляторов
  ...
  {
  	wheelsPidStruct[i].p_k = 2000.00; //1
  	wheelsPidStruct[i].i_k = 1.05; //2.05
  	wheelsPidStruct[i].d_k = 1; //0.5
  	wheelsPidStruct[i].pid_on = 1;
  	wheelsPidStruct[i].pid_error_end  = 3;
  	wheelsPidStruct[i].pid_output_end = 1000;
  	wheelsPidStruct[i].max_sum_error =50.0;
  	wheelsPidStruct[i].max_output = 500;
  	wheelsPidStruct[i].min_output = 20;
  }
}
