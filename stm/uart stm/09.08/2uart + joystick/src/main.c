...
int16_t speed[BUFFERSIZE]={};
int16_t speed1[BUFFERSIZE]={};
int s = 255;
int sp = 255;
char test;
...
void ADC_Configuration_new(void){
   ...
   ADC_RegularChannelConfig(ADC1,ADC_Channel_1,3,ADC_SampleTime_480Cycles);
...
int main(void) {
  ...
  TM_Delay_Init();
  init_USART1_1(26315*3.05); // initialize USART1 @ 9600 baud \\TC: Transmission complete
  init_USART6_1(26315*3.05);
  ...
  while (1)
  {
   ...
   //if (speed<40 & speed > -40){speed = 0;}

   //if (speed1<40 & speed1 > -40){speed1 = 0;}
    //TM_DelayMicros(100);
  }
}
...
/*	if( USART_GetITStatus(USART1, USART_IT_RXNE) ){
    ...
    { USART_ClearITPendingBit(USART1,USART_IT_TC);
        ...
        if (counter == 0)
        {   //speed = ((ADCConvertedValues[0]-2059) / 10);

            //speed[1] = speed[2];
            //speed[2]=speed[3];
            //speed[3]= ((ADCConvertedValues[1]-2055) / 10)+((ADCConvertedValues[0]-2055) / 10);
            USART_SendData(USART1,(~s)& 0xFF);
            //speed[0] = (speed[1] + speed[2] +speed[3]) / 3;
            speed[0]= 100;
            //speed = 150;
        }
        if (counter == 1)
...
void USART6_IRQHandler(void){
    ...
    {   USART_ClearITPendingBit(USART6,USART_IT_TC);
        if (counter1 == 0)
        {   //speed1 = -((ADCConvertedValues[1]-2055) / 10)+((ADCConvertedValues[0]-2055) / 10);
            //speed1[1] = speed1[2];
            //speed1[2]=speed1[3];
            //speed1[3]= -((ADCConvertedValues[1]-2055) / 10)+((ADCConvertedValues[0]-2055) / 10);
            //speed1[0] = (speed1[1] + speed1[2] +speed1[3]) / 3;
            speed1[0] = -100;
        ...
        if (counter1 == 3)
        {
            USART_SendData(USART6,speed1[0] & 0xFF);
        }
        if (counter1 == 4)
        {
            USART_SendData(USART6,(speed1[0] >> 8) & 0xFF);
        }
        if (counter1 == 5)
        {
            USART_SendData(USART6,speed1[0] & 0xFF);
